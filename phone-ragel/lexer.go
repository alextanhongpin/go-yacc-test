...
//line lexer.rl:40
( lex.p) = ( lex.te) - 1
{ lval.token = lex.string(); tok = OPERATOR; {( lex.p)++;  lex.cs = 5; goto _out } }
	goto st5
tr5:
//line lexer.rl:41
 lex.te = ( lex.p)+1
{ lval.token = lex.string(); tok = PHONE; {( lex.p)++;  lex.cs = 5; goto _out } }
	goto st5
tr7:
//line lexer.rl:38
 lex.te = ( lex.p)+1
{ lval.token = lex.string(); tok = int(lex.data[lex.ts]); {( lex.p)++;  lex.cs = 5; goto _out } }
	goto st5
tr9:
//line lexer.rl:37
 lex.te = ( lex.p)
...
//line lexer.rl:39
 lex.te = ( lex.p)
( lex.p)--
{ lval.token = lex.string(); tok = COUNTRY; {( lex.p)++;  lex.cs = 5; goto _out } }
	goto st5
tr12:
//line lexer.rl:40
 lex.te = ( lex.p)
